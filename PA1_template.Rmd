...
```
The mean value of steps per day is `r sprintf("%.2f",mymean)` and median is `r mymedian`.
...
```
The interval with maximum number of steps across all the days is `r max_int`.
...
```
There are `r n_nas` rows with missing values.
...
```
The mean value of steps per day after missing data imputation is `r sprintf("%.2f", mymean2)` and the median is `r sprintf("%.0f", mymedian2)`.  
